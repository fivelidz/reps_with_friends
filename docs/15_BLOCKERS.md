...
| T3 | **Bots only run while superlocal is on** — no always-on host. *(27 Aug: minirig install attempt blocked — box offline ~96d, Tailscale node key expired; needs power-on + `tailscale up --force-reauth` first, see docs/16)* | Demo reliability for Ben/Nico; "is the bot up?" becomes the story | Short-term: move bot processes to minirig (always-on, systemd user services — pattern exists). Long-term: small VPS or worker | 🟥 |
...
| T8 | ~~GitHub Actions runners not assigning~~ **RESOLVED — transient GitHub incident, not billing** (26–27 Aug): runs for e03bc09/ffba9fe queued forever (`runner_id: null`, no logs), but the very next push (5091488, 26 Aug 16:12) got a runner and deployed green; a fresh `workflow_dispatch` on 27 Aug also got a runner and succeeded in <60s. Stuck e03bc09 run is a harmless zombie (cancel API returns 409; ignore or delete via UI). | None — push-to-deploy works again; site verified live (200, /system fresh) | Nothing required. Watch item: repo is **private** so Actions minutes are paid — if it recurs, check github.com/settings/billing → raise spending limit, make repo public (Actions free), or self-host a runner on minirig | 🟩 |
