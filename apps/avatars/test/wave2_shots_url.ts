...
ws.onmessage = e => {
  ...
  if (m.method === 'Network.requestWillBeSent') pendingReqs.set(m.params.requestId, m.params.request.url + ' init=' + (m.params.initiator?.type ?? '?'));
  if (m.method === 'Network.loadingFailed') errors.push(`NETFAIL ${m.params.errorText} ${m.params.type ?? ''} url=${pendingReqs.get(m.params.requestId) ?? 'unknown'}`);
};
