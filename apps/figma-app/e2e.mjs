...
console.log("— POINTS + DRAFT (FLOW-05b)");
st = await state();
ok(st?.player?.points === 500, "identity starts with 500 points (trial currency, no money)");
ok(st?.matches?.[0]?.players?.every(p => Number.isFinite(p.points)), "every player carries a point wallet into the match");
ok(st?.matches?.[0]?.deadlineMode === "day", "day-mode deadline (no hard end in the create flow)");
await goto("#/battle-001");
ok(await exists(".fx-ptschip"), "points chip on the battle header");
ok(((await text(".fx-ptschip")) ?? "").includes("500 PTS"), "chip shows 500 PTS");
ok(await exists("#draftBtn"), "DRAFT READY badge (match-start draft open)");
ok(await exists(".fx-kittychip"), "kitty chip shown");
st = await state();
const draft0 = st.matches[0].drafts?.you;
ok(Array.isArray(draft0?.options) && draft0.options.length === 3, `pending draft holds 3 face-up options (${JSON.stringify(draft0?.options)})`);
ok(Object.keys(st.matches[0].drafts ?? {}).filter(k => k !== "you").length === 0, "mates auto-picked their start drafts (bots)");
const samInv0 = st.matches[0].inventory.sam?.length ?? 0;
ok(samInv0 >= 2, `mate inventory grew from the start draft (sam holds ${samInv0})`);
await click("#draftBtn");
ok(await exists("#draftSheet"), "draft sheet opens");
ok((await evalJs(`document.querySelectorAll('.fx-draftgrid .fx-draftcard').length`)) === 3, "3 face-up cards in the draft sheet");
ok(((await text("#rerollBtn")) ?? "").includes("50 PTS"), "reroll button shows the 50-pt first cost");
await shot("draft-sheet");
await click("#rerollBtn");
await sleep(300);
st = await state();
ok(st.matches[0].drafts?.you?.options?.length === 3, "reroll regenerated 3 options");
ok(st.matches[0].kitty?.points === 50, "reroll paid 50 POINTS to the kitty");
ok(st.matches[0].rerollCount?.you === 1, "reroll counter at 1 (escalation base 50→100→200)");
ok(st.player?.points === 450, `wallet synced after paying (500 → ${st.player?.points})`);
ok(st.player?.pointsLedger?.some(l => l.reason === "reroll" && l.delta === -50), "ledger tags the −50 reroll with a reason");
await goto("#/battle-001");
ok(((await text(".fx-kittychip")) ?? "").includes("KITTY 50"), "kitty chip shows 50 after the reroll");
st = await state();
const offered = st.matches[0].drafts.you.options[0];
await click("#draftBtn");
await evalJs(`(() => { document.querySelector('[data-pick="${offered}"]').click(); return true; })()`);
await sleep(300);
st = await state();
const picked = st.matches[0].inventory.you.find(i => i.kind === offered);
ok(!!picked, `picked card "${offered}" landed in your inventory`);
ok(Number.isFinite(picked?.expiresAt) && picked.expiresAt > Date.now(), "picked card carries a future expiresAt (cards expire)");
ok(st.matches[0].drafts?.you == null, "draft cleared after the pick");
await goto("#/pwr-001");
ok((await evalJs(`Array.from(document.querySelectorAll('.fx-exp')).length`)) >= 1, "expiry badges render on held cards");
ok(((await text(".fx-rule__v")) ?? "").includes("PTS"), "arsenal shows the points balance");
await shot("pwr-arsenal-v2");
...
ok((await evalJs(`document.querySelectorAll('.fx-pwrgrid .fg-pwr').length`)) === 14, "all 14 power-up kinds in the arsenal grid (v2 range)");
...
ok((await text(".fx-ltline--target"))?.includes("BLOCKED BY SHIELD"), "target preview flags the shield");
