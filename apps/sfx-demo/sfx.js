/* ═══════════════════════════════════════════════════════════════════════
   RWF SFX — SFX DEMO COPY of apps/figma-app/sfx.js (defensive, one file).

   Same synthesis engine the live app plays: lazy AudioContext, master gain
   0.3, mute persisted at localStorage `rwf.sfx.muted` (shared with the app —
   mute here mutes the app too), combo-climbing log pitch, and the identical
   CATALOGUE. Export: window.rwfSfx = { play, setMuted, isMuted, toggle,
   names } — same global API apps/board calls.

   DIVERGED FROM THE ORIGINAL (only the DOM wiring, never the sound design):
   the figma-app copy also installs a delegated click→sound mapper
   (.fx-btn/.fg-chip/… conventions), a battle-countdown ticker and a
   MutationObserver control painter. Those target app screens; on this demo
   page they would double-fire a "tap" under every button. This copy wires
   buttons itself (see index.html) and exposes play() for exactly that.

   Re-sync rule: if a sound changes in apps/figma-app/sfx.js, change the
   CATALOGUE + synth primitives here to match — byte-identical where marked.
   ═══════════════════════════════════════════════════════════════════════ */
...
/* (demo copy: paintControls/MutationObserver live in the app's copy — this
   page paints its own mute button; see index.html) */
function setMuted(v) {
  muted = !!v;
  persistMuted();
  return muted;
...
/* (demo copy: the app's paintControls/MutationObserver, delegated
   click→sound mapper and battle-ticker watchBattle live in apps/figma-app/
   sfx.js only — on this page they would double-fire a "tap" under every
   demo button. This page wires its buttons itself; see index.html.) */
