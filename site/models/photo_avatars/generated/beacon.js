// site/models/photo_avatars/generated/beacon.js — PHOTO AVATAR #4
//
// FIRST REAL vision-driven img2threejs run (docs/23 §5, 2026-09-02):
//   reference  : authored flat-colour test bust (scripts/img2threejs_run/
//                test_subject.png — PIL-composed, licence-clean, no people)
//   intake     : glm-4.6v (native v4 vision API) — palette 8/9 hex hits
//   correction : 1 host-feedback pass (skill refine-spec loop)
//   codegen    : glm-5.3 (anthropic shim), spec 18 components
//   gate       : headless render + pixel-probe + glm-4.6v review
// Method: reconstruction-by-code (img2threejs skill, Apache-2.0 contract).
// No meshes, no textures, no scan data. This file is ours.
...
export function createBeaconModel() {
  ...
  const fringeSweep = M(new THREE.SphereGeometry(0.55, 16, 12), C.hair, Fringe, [-0.30, 0.5, 0.68]);
  fringeSweep.scale.set(1.5, 0.42, 0.6);
  fringeSweep.rotation.z = 0.35;                                                        // covers half of LEFT eye
  // refine-code pass 1: knot lifted ABOVE the band crown (was colliding at
  // y≈1.58 with the band top ~1.16+0.25 → occluded, pixel gate caught it)
  const topKnot = M(new THREE.SphereGeometry(0.22, 12, 10), C.hair, TopKnot, [0, 1.80, 0.12]);
  topKnot.scale.set(1, 0.85, 1);                                                        // top-knot clears the band, above crown

  // ── zone: face — sky-blue rounded-rect eyes (z-squashed capsules) ────────
  // refine-code pass 1: z 0.95→1.06 (eyes were BURIED in the head sphere —
  // surface distance was 0.978 < r=1.0; only a sliver rendered)
  for (const sx of [1, -1]) {
  ...
  // refine-code pass 1: rings lost their gold under lights with no envMap
  // (metalness 0.45 rendered near-black) — de-metal + emissive lift + nudge
  // outward so they clear the cups
  const ring_L = M(new THREE.TorusGeometry(0.34, 0.05, 8, 24), C.gold, Headphones, [-1.24, 0.25, 0.02], { metalness: 0.1, roughness: 0.4, emissive: new THREE.Color(C.gold), emissiveIntensity: 0.25 });
  ring_L.rotation.y = Math.PI / 2;
  const ring_R = M(new THREE.TorusGeometry(0.34, 0.05, 8, 24), C.gold, Headphones, [1.24, 0.25, 0.02], { metalness: 0.1, roughness: 0.4, emissive: new THREE.Color(C.gold), emissiveIntensity: 0.25 });
