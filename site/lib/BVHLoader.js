// site/lib/BVHLoader.js — VENDORED from three.js r177
// (examples/jsm/loaders/BVHLoader.js, upstream commit tag r177). Unmodified
// except this header. Parses BVH mocap into { skeleton, clip }: a linked
// THREE.Bone hierarchy (position = joint OFFSET) plus an AnimationClip with
// `<bone>.position` / `<bone>.quaternion` tracks per joint.
// NOTE for this repo: the loader ADDS the joint offset to every position
// track value (delta semantics). Our BVHs export absolute frame positions,
// so the root reads out at 2x offset at frame 0 — harmless for the
// world-space retarget in site/model-avatars.js (BVHPlayer), which only
// uses position DELTAS between frames.
